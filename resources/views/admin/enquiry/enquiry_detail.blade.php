                ...
                <div id="enqDetail" class="main-content-body main-content-body-chat">
                    <div class="main-chat-header bg-primary" style="border-radius: 4px 4px 0 0;">
                        ...
                        <nav class="nav"> 
                            ...
                            <a class="btn btn-light pl-1 pr-1 btn-sm deleteBtn text-black" data-bs-toggle="tooltip" title="Delete Enquiry" rel1="delete-enquiry" rel="{{$enqDetail->id}}"><i class="fa fa-trash"></i></a>
                        </nav>
                    </div>
                            ...
                            <div class="ps-0">
                                <div class="main-profile-overview">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-3">
                                            <h6 class=""><span class="font-weight-normal">Status</span>:
                                                <button data-bs-toggle="dropdown" id="status{{$enqDetail->id}}" class="btn btn-sm">{{ $enqDetail->status }} <i class="icon ion-ios-arrow-down tx-11 mg-l-3"></i></button>
                                        			<div class="dropdown-menu box-shadow-primary">
														<!-- <option value="NULL">Select Status</option> -->
														<a class="dropdown-item" onclick="orderStatus('HOT',{{$enqDetail->id}})">Hot</a>
														<a class="dropdown-item" onclick="orderStatus('WARM',{{$enqDetail->id}})">Warm</a>
														<a class="dropdown-item" onclick="orderStatus('LONG LEAD',{{$enqDetail->id}})">Long Lead</a>
														<a class="dropdown-item" onclick="orderStatus('BUDGETARY',{{$enqDetail->id}})">Budgetary</a>
														<a class="dropdown-item" onclick="orderStatus('CANCELLED',{{$enqDetail->id}})">Cancelled</a>
														<a class="dropdown-item" onclick="orderStatus('HOLD',{{$enqDetail->id}})">Hold</a>
														<a class="dropdown-item" onclick="orderStatus('LOST',{{$enqDetail->id}})">Lost</a>
														<a class="dropdown-item" onclick="orderStatus('ORDER',{{$enqDetail->id}})">Order</a>
														<a class="dropdown-item" onclick="orderStatus('PO AWAITED',{{$enqDetail->id}})">Po Awaited</a>
													</div>
													</select>
                                            </h6>
                                        </div>
                                    ...
                                    <div class="row mb-2">
                                        <div class="col-md-3 col">
                                            <?php if(!empty($enqDetail->address)){?>
                                    ...
                                    @foreach($enqNotes as $enqnote)
                                        <div class="media mr-1 w-100" id="media{{$enqnote->id}}">
                                            ...
                                            <div class="media-body"><span class="tx-14-f">{{$enqnote->note}}</span><small><i class="fa fa-clock"></i>{{date('d M, Y', strtotime($enqnote->updated_at))}}</small></div>
                                            <div><a href="javascript:void(0)" class="text-black" onclick="deleteNote('{{$enqnote->id}}')"><i class="fa fa-trash"></i></a></div>
                                        </div>
                                    @endforeach
                                    <div id="addedNote"></div>
                                    <button aria-expanded="false" aria-haspopup="true" class="btn ripple btn-light m-1 pl-1 pr-1 btn-sm" data-bs-toggle="dropdown" id="dropdownMenuButton" type="button"><i class="fa fa-edit"></i> Add Note</button>
                                    <div class="dropdown-menu box-shadow-primary p-2 w-50">
                                        <div class="form-group m-0">
